// ignore_for_file: file_names

import 'package:dartz/dartz.dart';
import 'package:equatable/equatable.dart';
import 'package:hassy/Features/teacher/domain/entities/get_cities_entity.dart';
import 'package:hassy/Features/teacher/domain/repositories/base_teacher_repository.dart';
import 'package:hassy/core/utils/error/failure.dart';
import 'package:hassy/core/utils/usecase/base_use_case.dart';

class GetCitiesUseCase
    extends BaseUseCase<GetCitiesEntity, GetCitiesParameters> {
  BaseTeacherRepository baseStudentRepository;
  GetCitiesUseCase(this.baseStudentRepository);
  @override
  Future<Either<Failure, GetCitiesEntity>> call(
      GetCitiesParameters parameters) {
    return baseStudentRepository.getCities(parameters);
  }
}

class GetCitiesParameters extends Equatable {
  final String token;
  final int countryId;
  const GetCitiesParameters({
    required this.token,
    required this.countryId,
  });

  @override
  List<Object?> get props => [
        token,
        countryId,
      ];
}
